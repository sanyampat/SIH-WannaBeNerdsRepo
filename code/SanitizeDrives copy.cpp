...
struct DriveInfo {
    std::string name;   // Windows: "C:" (logical), Linux: "/dev/sda"
...
#ifdef _WIN32
// ---------------- Windows drive detection + mapping ----------------
#define NOMINMAX
#include <windows.h>
#include <winioctl.h>
#include <vector>
...
void sanitizeDrive(const DriveInfo& d, bool dryRun = false) {
    ...
    if (d.bus == "nvme") {
        ...
        // For SATA drives prefer ATA secure erase if supported
        ok = ataSecureErase(targetPath);
        if (!ok) {
            std::cerr << "[!] ataSecureErase failed or unsupported for " << targetPath << ", falling back to overwrite.\n";
